...
export default function GalleryPage() {
  ...
  const dotCount = Math.max(
    1,
    currentItems.length - visibleCards + 1
  );
  const visibleEventItems =
  currentSlide + visibleCards <= eventItems.length
    ? eventItems.slice(
        currentSlide,
        currentSlide + visibleCards
      )
    : [
        ...eventItems.slice(currentSlide),
        ...eventItems.slice(
          0,
          currentSlide +
            visibleCards -
...
const nextSlide = () => {
  setCurrentSlide((prev) =>
    prev >= eventItems.length - visibleCards
      ? 0
      : prev + 1
  );
};
        ...
        <div className="flex justify-center mb-2">
          ...
          <div className="flex rounded-full bg-black/30 p-1 backdrop-blur-sm">
            ...
            <button
              ...
              className={`px-5 py-2 rounded-full transition ${tab === "events"
            ...
            <button
              ...
              className={`px-5 py-2 rounded-full transition ${tab === "daily"
                  ? "bg-sky-400 text-white"
                  : "text-white/70"
                }`}
            >
            ...
            <button
              ...
              className={`px-5 py-2 rounded-full transition ${tab === "support"
                  ? "bg-sky-400 text-white"
                  : "text-white/70"
                }`}
            >
...
{visibleEventItems.map((item) => {
  const realIndex = eventItems.findIndex(
    (e) => e.src === item.src
  );
      ...
      <div className="px-3">
        ...
        >
          ...
          <div className="absolute bottom-2 left-0 right-0 p-6">
            <h2 className="text-sm font-semibold uppercase text-sky-300">
              {item.title}
            </h2>
          </div>
